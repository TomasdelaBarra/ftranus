...
! * 
! * $Id: trans.f90 138 2010-05-26 23:45:10Z juancarlo.anez $
! * 
...
!  ABRE LOS ARCHIVOS DE FLUJOS (F1S) -excepto SoloCostos- Y DE PASOS (P_S)
      ...
      ENDIF
      NPUB = 0 ! N�mero de modos p�blicos
      DO K=1,NTM
         ! Abre los archivos de pasos y calcula el n�mero de modos p�blicos
         IUNPAS(K)=K+100
...
! ABRE ARCHIVO CON CONTEOS
      ...
      ! No hay conteos en la iteraci�n anterior
      VOLANT = 0


!  COMIENZO DE ITERACIONES

      Puestos = -1 ! No usar en primera iteraci�n
...
! Blanquea indicadores de evaluaci�n
         DISTAN=RCERO     ! Unidades-km por categor�a
         TTIEMV=RCERO     ! Tiempo viaje total por categor�a
         TTIEME=RCERO     ! Tiempo espera total por categor�a
         CPROM=RCERO      ! Costo total por categor�a
         TOTV=RCERO       ! Viajes totales por categor�a y modo
         VIAJ=RCERO       ! Viajes totales por operador
         TUTIL=RCERO      ! Utilidad total por categor�a
         OPING=RCERO      ! Ingresos por operador
         COP=RCERO        ! Costo de operaci�n por operador
...
!  Iteraciones respecto a categorias IP
      ...
      DO IP=1,NPROP
        if (debugging >= dbg_normal) then
          WRITE(*,'(A1,1X,I3,2X,I3,1X,A8,'' ('',I2.2,''%)'',$)') &
...
!  Escribir resultados asignacion en T3S e indicadores en T2S, excepto SoloCostos
      ...
      IF(.NOT.SoloCostos)THEN
          ...
          !! call SaveCounts(counts_file)
      ELSE
         ! Dejar rastro de opci�n 'I' para que funcione el Makefile
...
!   -----------------------------------------------
      ...
      integer(4) :: CV, INF4
!-(j) convergencia por flujos
      LOGICAL :: PRIMERA = .TRUE.
!-(j)
      INTEGER L, LnR
...
!  Calcula los VehStd (VSTAN);  si son cero, no hay restric. de capac.
        ...
        ! convergencia por flujos
         IF(.NOT.PRIMERA)THEN
            IF(VSTAN.GT.CEROMAS) THEN
               CONV1=ABS(VSTAN-LnFlow(L))/VSTAN
            ENDIF
         ENDIF
...
!  la promedia con la de la iteracti�n ant (VEL)
         ...
         IF(CVEL.GT.CONVEL) THEN
            CONVEL=CVEL
            MAXVOR=IOR(L)
            MAXVDES=IDES(L)
         ENDIF

         Excess = (VSTAN+Queue(L))-CV
         Congest(l) = (Congest(l)+Excess)/2

      ENDDO  ! Fin L, para cada enlace
      ! Blanquear colas para asignaci�n
...
!  Recorre cada enlace-ruta para calcular la espera si es tipo 3
      DO LnR=1, NLINKRUT
         ...
         IF(ITIPOP(IO) == 3) then
              ...
              !Puestos vac�os por unidad de tiempo
              Q = Max(0., (TOC(io)*Veh(Lnr) - Passing)/FACTIEM(io))
              Puestos(LnR) = (Puestos(LnR) + Q)/2.

              W = EspMin(io)
              if (.not. RutScheduled(IR)) then
                    W = W + (1/(2*FreqNew(IR)) )
              endif
              W = WaitingTime(Suben(LnR), Puestos(LnR), W)
              if (W.lt.0) then
...
7328     IT=ITIP(L)
         ...
         !!!!! 
         ! Es incorrecto sumar el costo de operaci�n constante en cada enlace
...
!  Acumula los veh�culos en los enlaces
                  IF(ITIPOP(IO) /= 3 .and. ITIPOP(IO) /= 4) THEN
                       VEH(LnRa)=VEH(LnRa)+VV ! 0 acumulados para rutas. Se suman en la asignaci�n
                  !else los veh�culos se calculan durante la restricci�n de capacidad
                  ENDIF
